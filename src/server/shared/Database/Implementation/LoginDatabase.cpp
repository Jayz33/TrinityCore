...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(LOGIN_SEL_PINFO, "SELECT a.username, aa.gmlevel, a.email, a.last_ip, DATE_FORMAT(a.last_login, '%Y-%m-%d %T'), a.mutetime, a.realname FROM account a LEFT JOIN account_access aa ON (a.id = aa.id AND (aa.RealmID = ? OR aa.RealmID = -1)) WHERE a.id = ?", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(LOGIN_INS_ACCOUNT_SOCIAL, "INSERT INTO account_social (guid, friend) VALUES (?, ?)", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_SOCIAL, "SELECT friend FROM account_social WHERE guid = ? AND friend = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_SOCIAL_FRIEND, "SELECT friend FROM account_social WHERE guid = ?", CONNECTION_SYNCH);
